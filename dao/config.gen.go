...
type IConfigDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.Config, err error)
}

// FilterWithNameAndRole SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (c configDo) FilterWithNameAndRole(name string, role string) (result []model.Config, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, name)
	generateSQL.WriteString("SELECT * FROM config WHERE name = ? ")
	if role != "" {
